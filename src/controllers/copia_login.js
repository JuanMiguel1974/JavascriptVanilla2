...
export default () => {
    ...
    loginPage.querySelector("#login").addEventListener("click",
        function(event) {
            loginPage.querySelector("#formLogin").addEventListener("submit", function(event) {
                ...
                fetch("https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyD8hfH8YVdrzF4LkhLi4q5lKhVludOeG_k", {
                        method: "post",
                        headers: {
                        ...
                        if (response.ok) {
                            ...
                            setTimeout(() => { window.location.reload(); }, 998);
                            return response.json().then((text) => { console.log(text); throw new Error(text.error.message); });
                        }
                    })
                    .then((datos) => {
